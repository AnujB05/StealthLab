...
class Layer1Evaluator:
    def __init__(self, judge: PanelAgent, graph: Optional[GraphStore] = None):
        self.judge = judge
        self.graph = graph
    ...
    async def evaluate(self, candidate: Candidate, cited=None) -> Layer1Result:
        ...
        try:
            raw = await self.judge.respond(
                JUDGE_SYSTEM,
                build_judge_prompt(candidate.summary, candidate.rationale, ops_text),
            )
