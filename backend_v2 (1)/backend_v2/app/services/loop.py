...
from app.eval.layer2 import Layer2Result, SimulatedReplayEvaluator
...
class LoopOrchestrator:
    def __init__(
        ...
        judge: PanelAgent,
        layer2_agent: Optional[PanelAgent] = None,
        on_call=None,
    ):
        enforce_independence(judge, panel)
        self._pool = pool
        self._graph = GraphStore(pool)
        self._engine = DebateEngine(panel, on_call=on_call)
        self._evaluator = Layer1Evaluator(judge, self._graph, on_call=on_call)
        self._machine = DebateStateMachine(pool)
        # Layer 2 is optional: without an agent it's simply skipped, and
        # scorecards carry no empirical section rather than a fabricated
        # one. Passing the judge here would be a mistake -- an adjudicator
        # should not also be generating the evidence it adjudicates.
        self._layer2 = (
            SimulatedReplayEvaluator(pool, layer2_agent, on_call=on_call) if layer2_agent else None
        )
    ...
    async def _evaluate(
        ...
        for cand in eligible:
            ...
            # Layer 2 runs only on candidates that survive Layer 1 --
            # simulating replays for an argument already known to be
            # fallacious is wasted spend.
            layer2 = None
            if self._layer2 and layer1.passed:
                try:
                    task_row = await self._pool.fetchrow(
                        "SELECT name FROM task_nodes WHERE id = $1", task_node_id
                    )
                    layer2 = await self._layer2.evaluate(
                        cand.id, task_node_id, cand.change_set,
                        task_name=task_row["name"] if task_row else "",
                    )
                except Exception as exc:  # noqa: BLE001
                    # A Layer 2 failure must not lose a Layer 1 result --
                    # the scorecard is still useful without empirical data.
                    log.error("layer 2 evaluation failed for %s: %s", cand.id, exc)
            ...
            sc = Scorecard(
                ...
                layer1=layer1,
                layer2_tier=layer2.tier if layer2 and layer2.sufficient_data else None,
                layer2_metrics=layer2.to_metrics_dict() if layer2 else None,
                value_delivered=layer2.value_delivered if layer2 else None,
                blast_radius=blast,
                # Every op type is invalidate-and-append, so any approved
                # change can be walked back. Revisit if a destructive op
                # type is ever added.
                reversible=True,
                recommendation=_recommend(layer1, blast, layer2),
            )
    ...
    async def _persist_scorecard(self, sc: Scorecard) -> None:
        ...
        await self._pool.execute(
            ...
            "layer2_tier, layer2_metrics, value_delivered, "
            ...
            l1.unresolved_cites,
            sc.layer2_tier, sc.layer2_metrics, sc.value_delivered,
            sc.blast_radius, sc.reversible,
            sc.recommendation,
        )


def _recommend(layer1, blast_radius: int, layer2: "Layer2Result | None" = None) -> str:
    """
    ...
    if not layer2.sufficient_data:
        note += (
            " Empirical testing could not run: "
            + (layer2.notes[-1] if layer2.notes else "insufficient data")
        )
        return note

    better = [c.metric for c in layer2.comparisons if c.verdict == Verdict.BETTER]
    worse = [c.metric for c in layer2.comparisons if c.verdict == Verdict.WORSE]
    if worse:
        note += f" Simulated replay suggests a regression on: {', '.join(worse)}."
    if better:
        note += f" Simulated replay suggests improvement on: {', '.join(better)}."
    if not better and not worse:
        note += " Simulated replay found no detectable difference on any metric."
    # The tier caveat is non-negotiable: this is a model's opinion about a
    # counterfactual, and an approver reading a delta needs to know that.
    note += f" Evidence tier: {layer2.tier_label}, n={layer2.n_observations}."
    return note
