...
class DecompositionService:
    ...
    async def decompose(self, problem: str) -> Decomposition:
        ...
        try:
            raw = await self._generator.respond(DECOMPOSE_SYSTEM, user_prompt)
            payload = _extract_json(raw)
        except Exception as exc:  # noqa: BLE001
            log.error("decomposition generation failed: %s", exc)
            return Decomposition(
                feasible=False,
    ...
    async def _critique(self, result: Decomposition, problem: str) -> None:
        ...
        try:
            raw = await self._critic.respond(CRITIQUE_SYSTEM, user)
            payload = _extract_json(raw)
        except Exception as exc:  # noqa: BLE001
